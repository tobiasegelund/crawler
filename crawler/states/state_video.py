...
class VideoState(State):
    def download(self, max_size: int) -> None:
        if len(self.collection) == 0:
            logger.info(
                "[Info] No video files were found on the page. Try use --render to render javascript content, it might help identify video tags"
            )
        else:
            succes_ctr = 0
            for video in self.collection:
                try:
                    ...
                    with tqdm(
                        ...
                        with open(
                            self.context.save_dir.joinpath(video.name), "wb"
                        ) as f:
